...
const Header = () => {
 const [Iscroled , setiscorled] = useState(false)
//  console.log(Iscroled)
useEffect(()=>{
  const handelTab = () =>{
    if(window.scrollY > 0){
      setiscorled(true)
  ...
  window.addEventListener('scroll', handelTab);
  return ()=> window.removeEventListener('scroll', handelTab);
},[])

  return (
    <div className={`${Iscroled && 'bg-[#131313]'} fixed top-0 z-50 flex w-full items-center justify-between ` }> 
...
width="2500" height="677" viewBox="0.238 0.034 1024 276.742">
   ...
   <div className='flex space-x-4'>
    <Link className='style-name' to='/SCreenLogin'>Home</Link>
    <Link className='style-name' to='/SCreenLogin'>Tv Show</Link>
    <Link className='style-name' to='/SCreenLogin'>Movies</Link>
    <Link className='style-name' to='/SCreenLogin'>Recently</Link>
    <Link className='style-name' to='/SCreenLogin'>Addeed</Link>
    <Link className='style-name' to='/SCreenLogin'>My listed</Link>
   </div>

      </div>
<div className='flex items-center justify-center space-x-4 px-3 py-1 '>
 
<AiOutlineSearch  className=' w-7 h-6 cursor-pointer '/>
{/* <input type="text" placeholder='Search Name' onClick={()=>handelclick}/> */}

<AiOutlineBell className=' w-7 h-6 cursor-pointer' />

<img src="./images/logo/profile.png" className='w-[48px] rounded cursor-pointer'/>

<FaCaretDown className='w-7 h-6 cursor-pointer'/>
</div>
    </div>
  )
}
